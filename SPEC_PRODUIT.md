...
Les objectifs sont **définis au niveau de la journée**. L'objectif hebdomadaire est la somme des objectifs de chaque jour, pondérée par le coefficient OOO du jour (cf. section A.3.8) :
...
### A.3.4 Règles de catégorisation automatique

La catégorie d'un événement Google Calendar est déterminée selon les règles suivantes, évaluées dans l'ordre :

1. **OOO** — L'événement est de type "Out of Office" dans Google Calendar.
2. **Rendez-vous clients** — L'événement remplit l'une de ces conditions :
  - L'événement provient de Calendly (identifiable via les métadonnées Calendly) et est confirmé.
  - L'événement implique le propriétaire du calendrier et au moins un participant dont l'adresse e-mail n'est **pas** en `@goodvest.fr`.
3. **Interne** — L'événement remplit l'une de ces conditions :
  - Le titre de l'événement contient le mot "Interne".
  - L'événement a au moins un participant autre que le propriétaire du calendrier, **et** tous les participants ont une adresse e-mail en `@goodvest.fr`.
4. **Care** — *(règle de détection à définir selon les métadonnées disponibles)*
5. **Suivi leads** — N'est pas un événement calendrier réel. Calculé automatiquement à partir des créneaux libres (cf. section A.3.5).

Si aucune règle ne correspond, l'événement est catégorisé par défaut comme **Rendez-vous clients**.

### A.3.5 Calcul automatique des créneaux "Suivi leads"
...
### A.3.6 Durée effective des événements
...
### A.3.8 Événements OOO — Ajustement des objectifs
...
### A.3.10 Clic sur un rendez-vous client — Modal de détail
...
Les objectifs hebdomadaires sont dérivés : **objectif journalier × somme des coefficients OOO** de chaque jour de la semaine (cf. section A.3.8).
